...
########################################################################################################################
#####                                                                                                              #####
#####  Appendix S3 to Jucker et al. (2022). Tallo - a global tree allometry and crown architecture database.       #####                                        
#####                                                                                                              #####
#####  The R code below replicates the three case studies presented in Jucker et al. (2021). Please cite the       #####
#####  original paper if using this code in a publication. The code uses data from the Tallo Database              ##### 
#####  which is archived on Figshare and available for download here: xxx                                          #####   
#####                                                                                                              #####
########################################################################################################################
########################################################################################################################
...
setwd("C://Users//qn19665//OneDrive - University of Bristol//Projects//3. Side projects//Global drivers of tree allometry//Database paper//Jucker et al. GCB 2022 supporting information")
Tallo<-read.csv("Tallo.csv",header=T, na.strings=c("","NA"," ","NaN"))
Tallo_env<-read.csv("Tallo_env.csv",header=T, na.strings=c("","NA"," ","NaN"))
...
summary(M)
r.squaredGLMM(M)      # Rm = 0.46 Rc = 0.75
coef(M)
...
cor.test(slope_ci_h$slope_mean,slope_ci_h$aridity_index_m)    # rho = 0.55, P = 0.017
...
summary(M)
r.squaredGLMM(M)   # Rm = 0.55 Rc = 0.63
coef(M)
...
## Correlation between slope and aridity
cor.test(slope_ci_cr$slope_mean,slope_ci_cr$aridity_index_m)    # rho = 0.34, P = 0.16
...
summary(M)
r.squaredGLMM(M)         # Rm = 0.35 Rc = 0.56
coef(M)
...
## Correlation between slope and aridity
cor.test(slope_ci_crh$slope_mean,slope_ci_crh$aridity_index_m)    # rho = 0.24, P = 0.33
...
Tallo_hd<-merge(Tallo_hd,species_list[c(1,5)])      # 155333 trees
length(unique(Tallo_hd$species))                    # 343 species
...
table(new.data$division)                                                                # Angio = 304 species; Gymnos = 39 species
...
table(new.data$aridity_effect,new.data$division)[1,1]/table(new.data$division)[1]       # 26 species (9%) of angiosperms have significantly negative slopes
table(new.data$aridity_effect,new.data$division)[3,1]/table(new.data$division)[1]       # 140 species (46%) of angiosperms have significantly positive slopes
(table(new.data$aridity_effect,new.data$division)[4,1]+table(new.data$aridity_effect,new.data$division)[3,1])/
  table(new.data$division)[1]                                                           # 224 species (74%) of angiosperms have positive slopes
table(new.data$aridity_effect,new.data$division)[1,2]/table(new.data$division)[2]       # 10 species (26%) of gymnosperms have significantly negative slopes
table(new.data$aridity_effect,new.data$division)[3,2]/table(new.data$division)[2]       # 11 species (28%) of gymnosperms have significantly positive slopes
(table(new.data$aridity_effect,new.data$division)[4,2]+table(new.data$aridity_effect,new.data$division)[3,2])/
  table(new.data$division)[2]                                                           # 18 species (46%) of gymnosperms have positive slopes
...
cor.test(new.data$height_aridity_m,new.data$aridity_sp_mean)     # rho = 0.53, P < 0.0001

## Correlation between a species' height response to aridity (random slope for aridity_gc) and its mean aridity index value
cor.test(new.data$slope_aridity,new.data$aridity_sp_mean)        # rho = -0.18, P = 0.0006
...
setwd("C://Users//qn19665//OneDrive - University of Bristol//Projects//3. Side projects//Global drivers of tree allometry//Database paper//Jucker et al. GCB 2022 supporting information")
Tallo_env<-read.csv("Tallo_env.csv",header=T, na.strings=c("","NA"," ","NaN"))
current_climate<-read.csv("current_climate.csv",header=T, na.strings=c("","NA"," ","NaN"))
future_climate<-read.csv("future_climate_ssp245.csv",header=T, na.strings=c("","NA"," ","NaN"))
